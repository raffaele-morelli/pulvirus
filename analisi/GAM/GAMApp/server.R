...
shinyServer(function(input, output, session) {
    ...
    session$onFlushed(once = T, function() {
        output$aqMap <- renderLeaflet({
            ...
            map=leaflet::addCircleMarkers(map, data = no2Staz, lat = no2Staz$st_y, lng = no2Staz$st_x, group = "NO2",  radius = ptsRadius, 
                                          # color = pal(no2Staz$zona_climatica),
                                          color = "black", fill = FALSE,
                                          layerId = paste("no2", no2Staz$station_eu_code, sep = "_"),
                                          popup = paste0(
                                              "Location ID: ", no2Staz$station_eu_code,
                                              "<br> Name: ", no2Staz$nome_stazione,
                                              "<br> Type: ", no2Staz$zona_tipo,
                                              "<br> Lat: ", no2Staz$st_x,
                                              "<br> Long: ", no2Staz$st_y))

            map=leaflet::addCircleMarkers(map, data = pm10Staz, lat = pm10Staz$st_y, lng = pm10Staz$st_x, group = "PM10", radius = ptsRadius, 
            ...
            map=leaflet::addLayersControl(map,
                                          position = "topleft",
                                          baseGroups = c("Topo", "Satellite"), overlayGroups = c("NO2", "PM10", "PM25"),
    ...
    observeEvent(input$table_input_rows_selected, {
        lat = filter(stazioniAria, station_eu_code == reactive_objects$sel_station_eu_code) %>% select(st_y) %>% as.numeric()
        long = filter(stazioniAria, station_eu_code == reactive_objects$sel_station_eu_code) %>% select(st_x) %>% as.numeric()
        map_proxy %>% leaflet::setView(lng = long, lat = lat, zoom = 15)
    ...
    output$pollution_rose = renderPlot({
        ...
        if(nrow(gr) > 0) {
            ...
            polarPlot(gr, pollutant = c("value"), type = "reporting_year", cols = "jet", key.header = "--", key.footer = "--", key.position = "right")            
        }else{
            ggplot2::ggplot() + ggplot2::geom_blank() + annotate("text", x = 4, y = 25, label = "Non ci sono dati",  size = 9) + theme_void()
        }
    ...
    output$descrittive = DT::renderDataTable({
        ...
        p_funs <- map(p, ~partial(quantile, probs = .x, type = 8, na.rm = TRUE)) %>% 
        ...
        names(stat_yy)[5:11] <- c("Valore medio annuo", "sd", "min", "25th","mediana", "75th", "max")
        ...
        # validi per anno ####
        stat_yy2 <- stat_yy2 %>% mutate( n_validi = n-n_missing, percentuale_validi = n_validi/n )
    ...
    output$gam_output <- renderUI({
        ...
        modello <- paste("<h4>Modello</h4><b>", names(reactive_objects$models), "</b>")
    ...
    output$gam_summary <- renderUI({
        req(reactive_objects$models)
        sommario <- mgcv::summary.gam(reactive_objects$models[[1]][[1]]) 
    ...
    output$jd_plot <- renderPlot({
        ...
        plot(sm(bv, indx), allTerms = TRUE, select = 4) + geom_hline(yintercept = 0) + theme_pulvirus()
    })
    ...
    output$date_slider <- renderUI({
        ...
        date_min = min(2016)
        date_max = max(2020)
        sliderInput("date_slider", "Date range:", min = date_min, max = date_max, value = c(date_min, date_max), round = TRUE, sep = "")
    })    
